var data = `iyr:2015 cid:189 ecl:oth byr:1947 hcl:#6c4ab1 eyr:2026
hgt:174cm
pid:526744288
...
const fieldsValidation = {
  byr: (value) => parseInt(value) > 1920 - 1 && parseInt(value) < 2002 + 1, //: true, // required
  iyr: (value) => parseInt(value) > 2010 - 1 && parseInt(value) < 2020 + 1, //: true, // required
  eyr: (value) => parseInt(value) > 2020 - 1 && parseInt(value) < 2030 + 1, //: true, // required
  hgt: (value) =>
    (value.includes("cm") &&
      parseInt(value) > 150 - 1 &&
      parseInt(value) < 193 + 1) |
    (value.includes("in") &&
      parseInt(value) > 59 - 1 &&
      parseInt(value) < 76 + 1), //: true, // required
  hcl: (value) => new RegExp("^#[0-9a-f]{6}$").test(value), //: true, // required
  ecl: (value) => new RegExp("^(amb|blu|brn|gry|grn|hzl|oth)$").test(value), //: true, // required
  pid: (value) => /^\d{9}$/g.test(value), //: true, // required
  //"cid",//: false, // optional
};
...
data = data.split("\n\n").map((v) => {
  const arr = v
    .split(/\s|\n|\r/g)
    .reduce((acc, f) => ({ ...acc, [f.split(":")[0]]: f.split(":")[1] }), {});

  return fields.reduce((acc, f) => {
    if (f in arr) {
      console.log(f, arr[f], fieldsValidation[f](arr[f]));
    }
    return acc && f in arr && fieldsValidation[f](arr[f]);
  }, true);
});

console.log(data.filter((d) => d).length);
