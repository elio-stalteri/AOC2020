var data = `6-10 s: snkscgszxsssscss
6-7 b: bbbbbxkb
2-4 n: nnnjn
1-2 j: jjjj
5-9 z: jgzzzqhbj
...
data = data
  ...
  .map((v) => {
    const d = v.split(":");
    const pswd = d[1].trim();
    const letter = d[0].split(" ")[1];
    const counts = d[0].split(" ")[0].split("-");
    const min = parseInt(counts[0]);
    const max = parseInt(counts[1]);
    /*const regex = new RegExp(letter,"g");
  var count = (pswd.match(regex) || []).length;
  return count>=min && count<=max*/
    const letterArr = pswd.split("");
    if (
      (letterArr[min - 1] === letter || letterArr[max - 1] === letter) &&
      !(letterArr[min - 1] === letter && letterArr[max - 1] === letter)
    )
      console.log(letter, min, max, pswd);

    return (letterArr[min - 1] === letter || letterArr[max - 1] === letter) &&
      !(letterArr[min - 1] === letter && letterArr[max - 1] === letter)
      ? true
      : false;
  })
